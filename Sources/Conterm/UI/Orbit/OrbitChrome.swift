        ...
        if showHelp {
            ZStack(alignment: .top) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    HStack(spacing: 8) {
                        OrbitMark(color: Theme.textPrimary, size: 15)
                        Text("How Orbit works")
                            .font(Drop.title(15))
                            .foregroundStyle(Theme.textPrimary)
                        Spacer()
                        DropFilterChip(title: "Basics", selected: helpTabIndex == 0) {
                            withAnimation(Theme.Spring.snappy) { helpTabIndex = 0 }
                        }
                        DropFilterChip(title: "Keys", selected: helpTabIndex == 1) {
                            withAnimation(Theme.Spring.snappy) { helpTabIndex = 1 }
                        }
                        DropIconButton(symbol: "xmark", help: "Close") {
                            withAnimation(Theme.Spring.snappy) { showHelp = false }
                        }
                    }
                    .padding(.horizontal, OrbitPanel.inset + 2).padding(.top, 20).padding(.bottom, 10)
                    ScrollView {
                        VStack(alignment: .leading, spacing: 14) {
                            if helpTabIndex == 0 { helpBasics } else { helpKeys }
                        }
                        .padding(.horizontal, OrbitPanel.inset + 2)
                ...
                .orbitPanel(dim: 0.2, fadesEdges: true)
                .padding(.top, 76)
