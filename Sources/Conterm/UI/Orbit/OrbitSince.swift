...
extension OrbitOverlay {
    ...
    @ViewBuilder
    var dropSincePanel: some View {
        if !since.isEmpty {
            let shown = Array(since.prefix(4))
            VStack(alignment: .leading, spacing: 2) {
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    DropEyebrow("Since you looked away")
                    Spacer(minLength: 12)
                ...
                if since.count > shown.count {
                    Text("and \(since.count - shown.count) more")
                        ...
                        .padding(.leading, 22).padding(.top, 3)
                }
            }
            .padding(.bottom, 18)
    ...
    func dropSinceRow(_ c: OrbitChange) -> some View {
        HStack(spacing: 10) {
    ...
    func dropSinceTint(_ kind: OrbitChange.Kind) -> Color {
        switch kind {
        case .needsYou:   return Drop.warn
        case .taskFailed: return Drop.bad
        case .finished, .taskOk: return Drop.good
