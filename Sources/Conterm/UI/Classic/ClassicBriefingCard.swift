...
/// Classic interface style. Shared chrome for the briefing cards (Host Overview, Cluster
...
/// `ClassicBriefingPresenter`).
struct ClassicBriefingCard<Content: View>: View {
    let glassLive: Bool
...
struct ClassicBriefingPresenter<Item: Equatable, Card: View>: View {
    let item: Item?
    let onDismiss: () -> Void
