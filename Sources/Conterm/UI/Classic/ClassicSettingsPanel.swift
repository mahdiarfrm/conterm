...
/// Classic interface style. Settings as two detached panels — sidebar
/// navigation on the left, scrollable content on the right — that slide
/// in from the top like the Classic command palette. Reachable via ⌘, or
/// via the command palette ("Open settings").
struct ClassicSettingsPanel: View {
    ...
    /// Shared with the Liquid Drop panel, so a section requested by raw
    /// value (`AppState.requestedSettingsSection`) lands in either.
    typealias Section = SettingsPanel.Section
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            card {
                SettingsRow(title: "Interface",
                    ...
                    Picker("", selection: $prefs.interfaceStyle.withSound()) {
                        Text("Liquid Drop").tag(Preferences.InterfaceStyle.liquidDrop)
                        Text("Classic").tag(Preferences.InterfaceStyle.classic)
                    }
...
struct ClassicAboutContent: View {
    var body: some View {
